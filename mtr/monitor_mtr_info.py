...
# check last line
def check_last_line():
    command = r'tail -f -n 2 {0}'.format(temp_mtr_log)
...
def change_display_info(proto, package_loss, net_delay):
    ...
    print 'net-delay:{0}'.format(net_delay)
    print 'max-delay:{0}'.format(max_delay)
...
if __name__ == '__main__':
    ...
    output_data(ip)
    p = check_last_line()
    max_mtr_info = get_mtr_info()
    max_loss = max_mtr_info['max_loss']
    max_delay = max_mtr_info['max_delay']

    t = threading.Thread(target=main_func, args=(p,))
    t.start()
    mainloop()
