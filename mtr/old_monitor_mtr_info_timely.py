...
def save_abnormal_info(proto, package_loss, net_delay, ip_conflict):
    ...
    if proto == 'console' or package_loss >= max_loss or net_delay >= max_delay or ip_conflict:
        print 'abnormal'
        os.system(
            r'echo {0}, protocol:{1}, package_loss:{2}, net_delay:{3},ip_conflict:{4} >> {5}'.format(time_part, proto,
                                                                                                     package_loss,
                                                                                                     net_delay,
                                                                                                     ip_conflict,
...
Label(textvariable=var4, fg='blue', font=("黑体", 12)).grid(row=1, column=3, padx=10, pady=2)

color_display_status = {'proto_status': False, 'loss_status': False, 'delay_status': False, 'conflict_status': False}
...
def change_display_info(proto, package_loss, net_delay, ip_conflict):
    ...
    if ip_conflict:
        ...
        if conflict_status:
            ...
            Label(textvariable=var4, fg='Crimson', font=("黑体", 12)).grid(row=1, column=3, padx=10, pady=2)
        else:
            pass
...
def main_func():
    ...
    while True:
        ...
        save_abnormal_info(result_proto, result_package_loss, result_net_delay, result_conflict)
        change_display_info(result_proto, result_package_loss, result_net_delay, result_conflict)
